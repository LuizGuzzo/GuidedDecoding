...
from model.pixelformer.swin_transformer import SwinTransformer
from model.pixelformer.PQI import PSP
...
class PixelFormer(nn.Module):
    ...
    def forward(self, imgs):
        ...
        q3p = nn.PixelShuffle(2)(q3)
        q2 = self.sam3(enc_feats[2], q3p)
        q2p = nn.PixelShuffle(2)(q2)
        q1 = self.sam2(enc_feats[1], q2p)
        q1p = nn.PixelShuffle(2)(q1)
        q0 = self.sam1(enc_feats[0], q1p)
        bin_centers = self.bcp(q4)
        f = self.disp_head1(q0, bin_centers, 4)

        # import torchvision.transforms as vtransforms
        # vtransforms.ToPILImage()(enc_feats[3][0,0,:]).show()
        # vtransforms.ToPILImage()(q4[0,0,:]).show()        
        # vtransforms.ToPILImage()(q3[0,0,:]).show()

        # vtransforms.ToPILImage()(enc_feats[2][0,0,:]).show()
        # vtransforms.ToPILImage()(q3p[0,0,:]).show()
        # vtransforms.ToPILImage()(q2[0,0,:]).show()

        # vtransforms.ToPILImage()(enc_feats[1][0,0,:]).show()
        # vtransforms.ToPILImage()(q2p[0,0,:]).show()
        # vtransforms.ToPILImage()(q1[0,0,:]).show()

        # vtransforms.ToPILImage()(enc_feats[0][0,0,:]).show()
        # vtransforms.ToPILImage()(q1p[0,0,:]).show()
        # vtransforms.ToPILImage()(q0[0,0,:]).show()

        # vtransforms.ToPILImage()(f[0,:]).show()
