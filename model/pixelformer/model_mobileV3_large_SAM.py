...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.crf3 = SAM(input_dim=in_channels[3], embed_dim=crf_dims[3], window_size=win, v_dim=v_dims[3], num_heads=num_heads[3])
        self.crf2 = SAM(input_dim=in_channels[2], embed_dim=crf_dims[2], window_size=win, v_dim=v_dims[2], num_heads=num_heads[2])
        self.crf1 = SAM(input_dim=in_channels[1], embed_dim=crf_dims[1], window_size=win, v_dim=v_dims[1], num_heads=num_heads[1])
        self.crf0 = SAM(input_dim=in_channels[0], embed_dim=crf_dims[0], window_size=win, v_dim=v_dims[0], num_heads=num_heads[0])
